...
fun AppNavigation() {
    ...
    AnimatedNavHost(
        ...
        enterTransition = { fadeIn(tween(500)) + scaleIn(initialScale = 0.9f) },
        exitTransition = { fadeOut(tween(400)) + scaleOut(targetScale = 1.1f) },
        popEnterTransition = { fadeIn(tween(500)) + scaleIn(initialScale = 0.95f) },
        popExitTransition = { fadeOut(tween(400)) + scaleOut(targetScale = 1.05f) }
    ) {
        composable("login") {
            LoginScreen(
                onLoginSuccess = { usuario -> nombreUsuario = usuario; navController.navigate("categorias") },
                onRegisterClick = { navController.navigate("register") }
            )
        }
        composable("register") {
            RegisterScreen(
                onRegisterSuccess = { usuario -> nombreUsuario = usuario; navController.navigate("categorias") },
                onBackToLogin = { navController.popBackStack() }
            )
        }
        composable("categorias") {
            CategoriaScreen(nombreUsuario) { categoria -> navController.navigate("catalogo/$categoria") }
        }
        composable("catalogo/{categoria}") { backStackEntry ->
            val categoria = backStackEntry.arguments?.getString("categoria") ?: "Catálogo"
            CatalogoScreen(categoria, navController)
        }
        ...
        composable("pago_exitoso") { PagoExitosoScreen(navController) }
